...
bool Tracker::UpdateViewers(int iteration) {
  if (!viewer_ptrs_.empty()) {
    ...
    if (key == 'd') {
      ...
      std::cout << "\nInserisci il nomde di un body: ";
      std::getline(std::cin, inputString);
      std::cout << "\nInserisci il nomde di un secondo body: ";
      std::getline(std::cin, inputString2);

      for (auto &optimizer_ptr : optimizer_ptrs_) {
        for (auto &link_ptr : optimizer_ptr->ReferencedLinks()) {
          if (link_ptr->body_ptr()->name() == inputString) {
            for (auto &viewer_ptr : viewer_ptrs_) {
              if (viewer_ptr->camera_ptr()->name() == "color_camera") {
                start = link_ptr->body_ptr()->getCentroidIn2DFrame(
                viewer_ptr->camera_ptr()->intrinsics());
                flag = true;
              }
            }
          }
          if (link_ptr->body_ptr()->name() == inputString2) {
            for (auto &viewer_ptr : viewer_ptrs_) {
              if (viewer_ptr->camera_ptr()->name() == "color_camera") {
                end = link_ptr->body_ptr()->getCentroidIn2DFrame(
                viewer_ptr->camera_ptr()->intrinsics());
                flag2 = true;
              }
            }
      ...
      if (flag == false) {
        std::cout << "Non esiste il primo Body" << std::endl;
      }
      if (flag2 == false) {
        std::cout << "Non esiste il secondo Body" << std::endl;
      }
      if (flag == true && flag2 == true) {
        for (auto &viewer_ptr : viewer_ptrs_) {
          if (viewer_ptr->camera_ptr()->name() == "color_camera") {
            viewer_ptr->DrawLine(start, end,
                                 viewer_ptr->camera_ptr()->intrinsics());
          }
        }
      }
