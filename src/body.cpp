...
Eigen::Vector3f Body:: getCentroid() {
  // Calcolo il centroide usando la  struttura dati del modello.
  Eigen::Vector3f centroid(0, 0, 0);
  for (const auto &vertex : vertices()) {
    centroid += vertex;
  }
  centroid /= vertices().size();  // Calcolo della media
  return centroid;
}

Eigen::Vector3f Body::getCentroidIn2DFrame(const Intrinsics &intrinsics) {
  // Ottieni il centroide in coordinate locali
  Eigen::Vector3f centroid_local = getCentroid();

  // Trasformazione nelle coordinate del mondo
  Eigen::Vector4f centroid_world =
      body2world_pose() * Eigen::Vector4f(centroid_local[0], centroid_local[1],
                                       centroid_local[2], 1.0);

    // Proiezione nel piano immagine (frame 2D) utilizzando gli intrinsics
  float x_2d =
      (intrinsics.fu * centroid_world[0] / centroid_world[2]) + intrinsics.ppu;
  float y_2d =
      (intrinsics.fv * centroid_world[1] / centroid_world[2]) + intrinsics.ppv;

  // Restituisco le coordinate 2D nel frame immagine
  return Eigen::Vector3f(x_2d, y_2d, centroid_world[2]);
}
