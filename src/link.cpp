...
bool Link::AddChildLink(const std::shared_ptr<Link> &child_link_ptr) {
  ...
  std::cerr << "Child link " << child_link_ptr->name() << std::endl;
...
Eigen::Array3d Link::VisualizeCoordinates() {
  Eigen::Array3d coordinates;
  std::cout << "Body:" << body_ptr_->name() 
            << std::endl;
  std::cout << "Rotation matrix:\n"
            << body_ptr_->body2world_pose().rotation() << std::endl;
  std::cout << "Translation vector:\n"
            << body_ptr_->body2world_pose().translation().transpose()
            << std::endl;

  for (int i = 0; i < 3; i++) {
    coordinates[i] = body_ptr_->body2world_pose().translation().transpose()[i];
  }
