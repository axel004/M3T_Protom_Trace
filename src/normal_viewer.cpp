...
void NormalColorViewer::DrawLine(Eigen::Vector3f &start_point,
  ...
  float real_distance;
  float Z = 0.78;
  cv::Point2f punto_iniziale(start_point.x(), start_point.y());
  cv::Point2f punto_finale(end_point.x(), end_point.y());
  cv::Point2f punto_scritta(end_point.x() + 20, end_point.y()+ 20);

  cv::Scalar colore_segmento(0, 255, 0);
  cv::line(color_camera_ptr_->image(), punto_iniziale, punto_finale,
           colore_segmento, 2);
  distance = std::sqrt(std::pow(end_point.x() - start_point.x(), 2) +
                       std::pow(end_point.y() - start_point.y(), 2) 
                       + std::pow(end_point.z() - start_point.z(), 2));
  cv::putText(color_camera_ptr_->image(), std::to_string(distance),
              punto_scritta, cv::FONT_HERSHEY_SIMPLEX, 1,
              cv::Scalar(255, 255, 255), 5);
  real_distance = distance * (Z / intrinsics.fu);
  cv::imshow("Frame con segmento fisso", color_camera_ptr_->image());
}
...
void NormalDepthViewer::DrawLine(Eigen::Vector3f &start_point,
                                 Eigen::Vector3f &end_point,
                                 const Intrinsics &intrinsics) {
  float distance;
  ...
  std::cout << punto_iniziale << std::endl;
  std::cout << punto_finale << std::endl;
  cv::Scalar colore_segmento(0, 255, 0);
  cv::line(depth_camera_ptr_->image(), punto_iniziale, punto_finale,
           colore_segmento, 2);

  distance = std::sqrt(std::pow(end_point.x() - start_point.x(), 2) +
            std::pow(end_point.y() - start_point.y(), 2));
  cv::putText(depth_camera_ptr_->image(), std::to_string(distance),
              punto_finale, cv::FONT_HERSHEY_SIMPLEX, 5, cv::Scalar(255, 255, 255), 5);
  cv::imshow("Frame con segmento fisso", depth_camera_ptr_->image());
